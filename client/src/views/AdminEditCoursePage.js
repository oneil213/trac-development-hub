...
function AdminEditCoursePage(props) {
     
  let courseId = new useParams({}).id;
  const [currentUser, setCurrentUser] = useState(AuthService.getCurrentUser());
  const [isLoading, setIsLoading] = useState(true);
  const [alert, setAlert] = useState('Loading Course');
  const [currentCourse] = useState({ id: null, title : '', price : '', description : '', duration :  '', language : '', level : '', status : '', video : ''});
       ...
       <div className="wrapper">
        ...
        <div className="form-row">
          ...
          <FormGroup className="col-md-6">
            ...
            <Input value={course.status} onChange={onChange}  name='status' id="inputStatus" type="select">
